            ...
            <form method="POST" action="{{ route('update.product',$product->id) }}" enctype='multipart/form-data'>
              ...
              <div class="box-body">
                <div class="form-group">
                  ...
                  <input type="text" value="{{ $product->name }}" name="name" class="form-control @error('name') is-invalid @enderror" id="example1" placeholder="Nhập sản phẩm">
                    @error('name')
                ...
                <div class="form-group">
                  ...
                  <input type="number" value="{{ $product->quantity }}" name="quantity" class="form-control @error('quantity') is-invalid @enderror" id="example2" placeholder="Nhập số lượng sản phẩm">
                    @error('quantity')
                        <span class="message-err" style="color:red;">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                  ...
                  <input type="text" value="{{ $product->price }}" name="price" class="form-control @error('price') is-invalid @enderror" id="example3" placeholder="Nhập giá sản phẩm">
                    @error('price')
                        <span class="message-err" style="color:red;">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                  ...
                  <input type="text" min="0" value="{{ $product->sale_price }}" name="sale_price" class="form-control @error('sale_price') is-invalid @enderror" id="example4" placeholder="Nhập khuyến mãi">
                    @error('sale_price')
                        <span class="message-err" style="color:red;">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" name="category_id">
                    ...
                    <option  value="{{ $value->id }}" selected="{{ $product->category_id == $value->id ? 'selected' : '' }}" >{{ $value->name }}</option>
                    @endforeach
                  </select>
                </div>
                ...
                <div class="form-group">
                  ...
                  <div class="imageFile" style="height:120px;width:100%;">
                    <label for="file-upload" class="custom-file-upload">
                      <img src="{{url('images')}}/{{$product->image}}" id="image" style="height:100%;border-radius:5px;" style="" alt="">
                    </label>
                  </div>
                ...
                <div class="form-group">
                  ...
                  <label for="files" style="display:block;height:120px">
                    <div class="image-preview " style="background-image:url(http://127.0.0.1:8000/assest/images/upfile.JPG)">
                           @foreach($imageProduct as $value) 
                              <img src="{{ url('images') }}/{{$value}}">
                           @endforeach 
                    </div>

                    <div class="image-preview dn" style="background-image:url({{ url('assest') }}/images/upfile.JPG)"></div>
                  </label>
                ...
                <div class="form-group">
                  ...
                  <textarea  name="description" id="content" >{{ $product->description }}</textarea>
...
$("#files").on('change', function () {
  ...
  Showimg[0].classList.add('dn');
  Showimg[1].classList.remove('dn');
